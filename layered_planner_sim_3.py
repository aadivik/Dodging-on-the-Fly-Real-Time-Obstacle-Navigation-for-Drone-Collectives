...
class Params:
    def __init__(self):
        ...
        self.num_robots = 5 # number of robots in the formation
...
# obstacles = [#             # narrow passage#               np.array([[-2.5, -0.5], [-passage_location-passage_width/2., -0.5], [-passage_location-passage_width/2., 0.5], [-2.5, 0.5]]),
...
if __name__ == '__main__':
    ...
    while True: # loop through all the setpoint from global planner trajectory, traj_global
        ...
        for p in range(len(followers_sp)): # formation poses correction with local planner
            ...
            obstacles1 = np.array(obstacles + robots_obstacles) # combine exisiting obstacles on the map with other
for i, robot in enumerate(robots):
    if i != p:
        # Follower robot's position correction with the local planner
        robots[i].local_planner(obstacles1, params)
        followers_sp[i] = robots[i].sp
        # centroid pose:
        centroid = 0
