...
def move_obstacles(obstacles, params):
     # Adjust obstacle movement as needed
    obstacles[-3] += np.array([-0.02, -0.01]) * params.drone_vel
    obstacles[-2] += np.array([-0.008, 0.008]) * params.drone_vel/2
    obstacles[-1] += np.array([0.0, 0.01]) * params.drone_vel/2
    return obstacles
...
class Params:
    def __init__(self):
        ...
        self.influence_radius = 0.2 # [m] potential fields radius, defining repulsive area size near the obstacle
...
obstacles = [
    ...
    np.array([[0.5, 0], [2.5, 0], [2.5, 0.3], [0.5, 0.3]]),#[0.5, 0],
    np.array([[-2, -2], [-0.5, -2], [-0.5, -1.8], [-2, -1.8]]),
    np.array([[-0.7, -1.8], [-0.5, -1.8], [-0.5, -0.8], [-0.7, -0.8]]),

    # Triangular obstacle
    np.array([[2, 2], [2.5, 2.5], [2, 2.5]]),

    # Complex polygon obstacle
    np.array([
    [0.5, -1.2],   # Top center
    [0.25, -0.7],  # Top left
    [0.75, -0.7],  # Top right
    #[0, 0],      # Bottom left
    #[1, 0],      # Bottom right
    #[0.25, -1.5],  # Bottom left
    #[0.75, -1.5]   # Bottom right
]),
]
...
for i in range(100):
    angle = i * 2 * np.pi / 100
    x = 1.5 + 0.2 * np.cos(angle)
    y = 0.5 + 0.2 * np.sin(angle)
    circle_points.append([x, y])

# Add circular obstacle to the list of obstacles
obstacles.append(np.array(circle_points))
